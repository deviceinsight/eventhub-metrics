...
type Service interface {
	ProcessNamespace(ctx context.Context, credential *azidentity.DefaultAzureCredential, endpoint string) (string, []string, error)
	ProcessEventHub(ctx context.Context, credential *azidentity.DefaultAzureCredential, blobStore *checkpoints.BlobStore, namespace, endpoint, eventHub string) error
}
...
func (s *service) ProcessNamespace(ctx context.Context, credential *azidentity.DefaultAzureCredential, endpoint string) (string, []string, error) {
...
func (s *service) ProcessEventHub(ctx context.Context, credential *azidentity.DefaultAzureCredential, blobStore *checkpoints.BlobStore, namespace, endpoint, eventHub string) error {
	...
	for partitionID, seq := range sequenceNumbers {
		if err := s.metrics.RecordEventhubPartitionSequenceNumber(namespace, eventHub, partitionID, seq.Min, seq.Max); err != nil {
	...
	for _, consumerGroup := range consumerGroups {
		...
		checkpoints, err := blobStore.ListCheckpoints(ctx, endpoint, eventHub, consumerGroup, nil)
		if err != nil {
		...
		for _, checkpoint := range checkpoints {
			...
			if err := s.metrics.RecordConsumerGroupPartitionLag(namespace, eventHub, consumerGroup, checkpoint.PartitionID, lag); err != nil {
